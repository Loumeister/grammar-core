...
The public discovery interface is `/.agents/skills/<name>/SKILL.md`. Each skill also has `/.agents/skills/<name>/agents/openai.yaml` for Codex UI metadata.
...
The plugin marketplace is a complementary Claude Code distribution channel. Git subtree makes canonical `/.agents/skills/` physically visible as `/shared/grammar-core/.agents/skills/` in product repositories. The plugin-distributed sync skill is not a seventh canonical domain skill.
